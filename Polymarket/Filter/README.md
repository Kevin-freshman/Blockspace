...
- **迁移 CSV**：排行榜研究页按当前 Top K 导出进入、留榜和掉榜明细。
...
页面顶部的 **排行榜研究** 是独立于手动筛选的研究视图。服务会自动执行以下只读流程：

```text
每 6 小时保存 CRYPTO DAY 的官方 PNL Top 1,000 + VOL Top 1,000
    → 只比较相隔 24 小时且两边完整的快照
    → 标记进入 / 留榜 / 掉出 Top 100、500 或 1,000
    → 对重点掉榜地址用官方 user 参数复核其当前 PNL 排名
    → 每天 00:00 UTC 富集成交量匹配的重点 cohort
    → Finding 满足跨日证据门槛后才升级为 Insight
```

加入 VOL 榜是为了建立真实的高成交量输家对照池：VOL 排名返回同一地址的官方 PNL，
其中可以包含负值。这里的“榜外”始终表示“不在所选 PNL Top K”，不表示地址没有
PNL，也不表示账户消失。

每日富集最多从掉榜、留榜对照、进入榜单、高成交量输家和成交量匹配赢家各取 30 个
地址，计算公开可见的策略指纹：成交频率、每个交易哈希的 fill 数、成交规模、市场数与
集中度、BUY 占比、平均价格、价格不低于 0.90 的 BUY 占比，以及可由短周期 slug 推导
的结束前 60 分钟成交占比。每个地址最多读取两页活动，并抽取最多两个近期交易哈希做
Polygon 主网成功回执和 Polymarket 合约命中核验；不解码 maker/taker，也不据此给地址
贴“对敲”标签。

研究状态分为：

- `COLLECTING`：尚无一次每日富集；
- `FINDING`：已有描述性差异，但未达到外推门槛；
- `INSIGHT`：至少 7 个完整日样本、每组至少 30 个唯一地址、方向一致度至少 75%，且
  每日中位差的确定性 bootstrap 95% 区间不跨 0；
- `反直觉信号` 或 `证据不一致`：结果稳定地背离假设，或不同日期方向不一致。

官方没有历史排行榜接口，因此上线前的数据不能回填。第一个 6 小时快照完成后页面会
显示采集中；首个掉榜比较最早在 24 小时后出现，首个 Insight 最早在 7 个完整日样本后
出现。快照以压缩 JSON 原子写入 `data/research/`，保留 90 天；该目录不会被静态服务。

重要边界：官方 DAY PNL 是榜单窗口值，两个快照的差额不是“这 24 小时新增 PNL”。
挂单创建/撤单时刻、真实撮合延迟、完整库存路径与私有机器人参数在第一版不可观察。
POLITICS 跨品类资金假设和 maker/taker 对手方/疑似对敲研究也不在第一版范围内。
...
Filter/
  ...
  research_engine.py     # 榜单迁移、cohort、参数指纹与 Insight 门槛
  research_store.py      # data/research 压缩快照的原子存储与保留策略
  config.json            # 默认参数
  static/                # 无框架网页
  tests/                 # 离线单元测试
  deploy/                # systemd unit
